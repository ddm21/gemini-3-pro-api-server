...
class TestAuthentication:
    ...
    def test_generate_with_valid_api_key(self, client, sample_generate_request, valid_api_key, mock_gemini_client):
        ...
        # May fail due to actual Gemini API call, but should not be 403
        assert response.status_code != 403
...
class TestRateLimiting:
    ...
    def test_rate_limit_exists(self, client, valid_api_key, sample_generate_request):
        """Test that rate limiting is configured (won't hit limit in test)."""
        # Just verify the endpoint works - actual rate limit testing
        # would require many requests which is slow
        response = client.post(
            "/generate",
            json=sample_generate_request,
            headers={"X-API-Key": valid_api_key}
